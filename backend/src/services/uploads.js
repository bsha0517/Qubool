 ...
 *
 * Flow:
 *   1. Client calls POST /uploads/signed-url -> gets { uploadUrl, publicUrl, key }
 *   2. Client PUTs the file bytes directly to uploadUrl
 *   3. Client tells our API the publicUrl (see routes/profile.js)
...
const s3 = isConfigured ? new S3Client({ region: process.env.AWS_REGION || "me-south-1" }) : null;
const BUCKET = process.env.S3_BUCKET_NAME;

const ALLOWED_PURPOSES = ["profile-photo"];
const ALLOWED_TYPES = ["image/jpeg", "image/png", "image/webp"];

// `selfBaseUrl` should be derived from the actual incoming request
// (req.protocol + req.get("host") — see routes/uploads.js) rather than
// hardcoded, since a fixed env var default of localhost:PORT only ever
// works when the client and server happen to be on the same machine (true
// for local Docker, false for literally every real deployment). The env
// var is kept as a fallback for setups that explicitly want to override it
// (e.g. serving uploads from a different domain than the API itself).
async function getSignedUploadUrl({ userId, purpose, contentType, selfBaseUrl }) {
  ...
  if (!isConfigured) {
    const base = process.env.SELF_BASE_URL || selfBaseUrl || `http://localhost:${process.env.PORT || 4000}`;
    ...
    return {
      uploadUrl: `${base}/uploads/local/${encodeURIComponent(key)}?token=${uploadToken}`,
      publicUrl: `${base}/uploads/static/${key}`,
      key,
    };
  }
